...
from diffusers import AutoencoderKL, DDPMScheduler, DiffusionPipeline, UNet2DConditionModel, AutoPipelineForText2Image, AutoencoderTiny
...
from utils.se_parallel_utils import register_se_forward  # share encoder, parallel multi-step decoder
from utils.dataset import TextPromptDataset
...
@torch.no_grad()
def inference(vae, unet, noise_scheduler, encoded_embeds, generator, device, weight_dtype, register_store):
    ...
    pred_original_sample, _ = predict_original(unet, noise_scheduler, input_noise, prompt_embed, register_store)
...
def kl_divergence(noise, lambda_kl=0.0):
    """
    ...
    if lambda_kl > 0.0:
        for n in noise:
            ...
            kl_div = torch.sum(1 + logvar - mu.pow(2) - logvar.exp()) * (-0.5 * lambda_kl)
            total_kl_div += kl_div

    return total_kl_div

def main(args):
    args.output_dir = f"{args.output_dir}-step{args.num_inference_steps}"
    train_data_paths = {'JDB': 'JourneyDB/train/train_anno.jsonl'}
    train_data_path = train_data_paths[args.dataset]
    ...
    vae_tiny = AutoencoderTiny.from_pretrained(args.vae_tiny_name_or_path)  # madebyollin/taesd
    ...
    unet = UNet2DConditionModel.from_pretrained(
        args.student_pretrained_model_name_or_path, subfolder="unet", revision=args.revision
    )

    # Freeze vae, text encoders and teacher.
    vae_tiny.requires_grad_(False)
    ...
    register_store = {'se_step': None, 'skip_feature': None, 'mid_feature': None, 'lora_scale': None,
                      'use_parallel': False, 'timesteps': [], 'bs': args.train_batch_size}

    register_store['use_parallel'] = args.use_parallel
    register_se_forward(unet, register_store)
    ...
    vae_tiny.to(accelerator.device, dtype=weight_dtype)  # 367MB
    vae.to(accelerator.device, dtype=torch.float32)  # 717MB
    text_encoder.to(accelerator.device, dtype=weight_dtype)  # 620MB
    ...
    with accelerator.main_process_first():
        train_dataset = TextPromptDataset(train_data_path)
    ...
    null_dict = encode_prompt([""], text_encoder, tokenizer)  # 362MB
    ...
    def collate_fn(examples):
        ...
        prompt = [example["prompt"] for example in examples]  # for sdxlturbo
    ...
    if accelerator.is_main_process:
        ...
        accelerator.init_trackers("loopfree", config=tracker_config)
    ...
    def get_clip_weight(step, total_steps, initial_weight=.1, final_weight=.0):
        weight = initial_weight - (initial_weight - final_weight) * (step / total_steps)
        return max(final_weight, weight)
    total_training_steps = train_dataloader.total_dataset_length / args.train_batch_size
    total_training_steps = int(-(-total_training_steps // 1))
    initial_clip_weight = 0.1
    final_clip_weight = 0.0
    clip_model = CLIPModel.from_pretrained(args.clip_pretrained_model_name_or_path).to(accelerator.device)
    clip_processor = AutoProcessor.from_pretrained(args.clip_pretrained_model_name_or_path)
    clip_model.eval()
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet, teacher_lora):
                ...
                pred_original_samples, noise_pred = predict_original(unet, scheduler, input_noise, prompt_embeds, register_store)
                pred_original_samples = pred_original_samples.to(dtype=weight_dtype)
                noise_pred = noise_pred.to(dtype=weight_dtype)

                ## ---------- CLIP loss ---------- ##
                # CLIP loss
                textclip_loss = torch.tensor(.0).to(accelerator.device)
                if args.lambda_textcliploss > 0:
                    student_samples = pred_original_samples / vae_tiny.config.scaling_factor
                    student_samples = vae_tiny.decode(student_samples.to(dtype=vae_tiny.dtype))[0]

                    def clip_features(samples):
                        samples = ((samples + 1) / 2).clamp(0, 1)
                        # inputs = clip_processor(images=samples, return_tensors="pt").to(accelerator.device)
                        inputs = BatchFeature()
                        samples = F.interpolate(samples, size=(224, 224), mode='bicubic', align_corners=False)
                        inputs.data = {"pixel_values": samples}
                        features = clip_model.get_image_features(**inputs)
                        return features

                    featclip_student = clip_features(student_samples)
                    featclip_student = featclip_student / featclip_student.norm(dim=-1, keepdim=True)

                    current_clip_weight = get_clip_weight(global_step, total_training_steps, initial_clip_weight,
                                                          final_clip_weight)
                    ...
                    if args.lambda_textcliploss > 0:
                        with torch.no_grad():
                            def textclip_features(prompts):
                                inputs = clip_processor(text=prompts, return_tensors="pt", padding=True,
                                                        truncation=True).to(accelerator.device)
                                features = clip_model.get_text_features(**inputs)
                                return features
                        feattextclip = textclip_features(batch["prompt"])
                        feattextclip = feattextclip / feattextclip.norm(dim=-1, keepdim=True)

                        tau = args.lambda_textcliploss
                        textclip_loss = torch.nn.functional.relu(tau - torch.mean(cos(feattextclip, featclip_student)))
                ## ---------- CLIP loss ---------- ##

                # KL loss
                kl_loss = kl_divergence(noise_pred, args.lambda_kl)
                ...
                LOSS_VSD = loss_vsd.clone().detach()

                # CLIP loss
                if args.lambda_textcliploss > 0:
                    loss_vsd += current_clip_weight * textclip_loss
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 0 or global_step == args.max_train_steps or global_step in [0,]:
                    ...
                    if global_step % args.validation_steps == 0 or global_step == args.max_train_steps or global_step == 1:
                        if args.validation_prompts is not None and args.num_validation_images > 0:
                            ...
                            register_store['bs'] = args.train_batch_size
            ...
            logs = {
                "step_loss_vsd": LOSS_VSD.item(),  # loss_vsd.detach().item(),
                "step_loss_textclip": textclip_loss.detach().item(),
